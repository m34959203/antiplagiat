...
async def create_check(
    ...
    logger.info(f"📝 New check: {task_id} | mode={request.mode} | len={len(request.text)}")
    ...
    logger.info(f"✓ Check {task_id}: {duration:.2f}s | {result.get('originality')}% | matches={len(result.get('matches', []))}")
